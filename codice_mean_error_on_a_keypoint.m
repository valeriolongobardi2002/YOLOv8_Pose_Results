...
for v = 1:length(versions)
    ...
    [confidence, keypoints_pred, keypoints_true, missed_detection, false_detection] = function_estrazione_dati_singolo_key_CONF(versions{v}, ...
        'C:\Users\HP\OneDrive\Desktop\TESI UNI\testing\Labels_KRN_seq_1_resize', 0.98);  
...
subplot(3, 1, 3);
plot(1:3680, ERR_x(3,:), '-r');
hold on;
plot(1:3680, ERR_y(3,:), '-b');
hold off;
title('YOLOv8 NANO');
xlabel('Frame');
ylabel('Error (pix)');
...
for i=1:length(versions)
    ...
    versions_name={'MEDIUM', 'SMALL', 'NANO'};
    title(['YOLOv8 ' versions_name{i}]);
    xlabel('X-axis error (pix)');
    ylabel('Number of occurances');
    subtitle(['Mean = ' num2str(mediaX(i)) ' Standard deviation = ' num2str(std_x(i)), ' Missed/false and conf<95% detections: ', num2str(WRONG(i))]);
    ...
    ylabel('Number of occurances');
    subtitle(['Mean = ' num2str(mediaY(i)) ' Standard deviation = ' num2str(std_y(i)), ' Missed/false and conf<95% detections: ', num2str(WRONG(i))]);
    j=j+1;
end
